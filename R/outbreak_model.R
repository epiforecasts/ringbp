...
secondary_draw <- function(index_case, r0isolated, r0community, disp.iso, disp.com,prop.ascertain) {
  ...
  if (nrow(new_cases) > 0) {
    ...
    new_cases <- map(new_cases,~mutate(data.frame(exposure = rep(.$latent, times = .$new.cases),
                                       ...
                                       isolated_time = ifelse(vect_isTRUE(missed),onset + rep_fn(.$new.cases),
                                                              .$isolated_time + rep_fn(.$new.cases)),isolated = !vect_isTRUE(missed)))
