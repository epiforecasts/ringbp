...
outbreak_model<-function(nrun,initc, initclust,r0Amiss,r0Cmiss,r0A,r0B,pr.id,wvacc,massvac=0,chain.output=F,vacc_cf=F,
  ...
  #   r0Ba=r0B
  # }else{r0Ba=r0A}
  if(wvacc==0){r0_isolated_case <- r0_ring_pre_isolation} # if isolation off, r0 of isolated cases is same as those in ring

  initial.cases.pcluster <- initc
  initial.clusters <- initclust
  r0_missed_case <- r0Amiss
  r0_isolated_case <- r0B
  r0_ring_pre_isolation <- r0A
  prop.ascertain <- pr.id

  # r0Ba <- r0_isolated_case
  # r0Ba <- r0_ring_pre_isolation
  ...
  while(nclusters > 0 & clustindex <= nclusters & latest.onset < cap_max_days & length(onset.store.tab) < cap_cases) { # Start cluster loop
    ...
    while(ncases > 0 & ncases < ring.size) {
      ...
      #   secondary <- offspring(tcases,hc.vacc,r0mainA=r0_ring_pre_isolation,r0Bmain=r0_isolated_case,r0Amiss=r0_missed_case,
      #                          r0Cmiss,index.ascertained,tvacc,overkk,overkkmiss,ncases,first.case,vacc_cf=T,vacc_cf_index=T)
      # }
      #
      # # In vaccine counterfactual, generate cases without vaccine reduction
      # if(clustindex<=initclust & vacc_cf==T & tcases[1] !=cluster_onset.inf){
      #   secondary <- offspring(tcases,hc.vacc,r0mainA,r0Bmain,r0Amiss,
      #                          r0Cmiss,index.ascertained,
      ...
      if(clustindex>initclust | vacc_cf==F){
        secondary <- offspring(x = tcases,
                               hc.vacc = hc.vacc,
                               r0A = r0_ring_pre_isolation,
                               r0B = r0_isolated_case,
                               r0Cmiss = r0Cmiss,
                               r0Amiss=r0_missed_case,
                               index.ascertained = index.ascertained,
                               tvacc = tvacc,
                               overkk = overkk,
                               overkkmiss = overkkmiss,
                               ncases = ncases,
                               first.case = first.case,
                               vacc_cf=T,vacc_cf_index=T)
      }
