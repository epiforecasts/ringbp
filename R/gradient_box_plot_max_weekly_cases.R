#' @export
gradient_box_plot_max_weekly_cases <- function(
  results = NULL, cap_cases = 5000, extinct_thresold = 0.5,
  low_color = "lightblue", high_color = "darkblue") {
  ...
  ## make plot
  plot <- filt_results %>%
    dplyr::filter(control_effectiveness >= 0.6) %>%
    ...
    ggplot2::scale_color_gradient(low = low_color, high = high_color, guide="none")
