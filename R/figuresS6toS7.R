...
make_figure_S6 <- function(){
  ...
  inc_df <- data.table::data.table(x=seq(0,20,0.01),y=dweibull(seq(0,20,0.01),shape=2.322737,scale=6.492272))
  ...
  tab <- tab %>% dplyr::mutate(theta=factor(k,levels = c(30,1.95,0.7),
                                     labels = c("<1%","15%","30%")))

  tab_sm <- tab %>% dplyr::group_by(theta) %>% dplyr::summarise(mean=mean(samp))

  tab %>%
    ggplot2::ggplot(aes(x=samp,fill=as.factor(theta),col=as.factor(theta))) +
    ggplot2::geom_density(alpha=0.2) +
    ggplot2::theme_bw() +
    ggplot2::geom_vline(data=tab_sm,aes(xintercept=mean,col=as.factor(theta)),lty=2) +
