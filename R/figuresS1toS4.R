...
make_figure_S1 <- function(res=NULL){
  ...
  ic1 <- sub_plotS1(num.initial.cases = 5,facet.by = "num.initial.cases")
  ic2 <- sub_plotS1(num.initial.cases = 40,facet.by = "num.initial.cases")

  # Delay distribution
  index_dl <- sub_plotS1(facet.by = "delay")
  dl1 <- sub_plotS1(delay = "Wuhan",facet.by = "delay")

  # theta
  index_th <- sub_plotS1(facet.by = "theta")
  th1 <- sub_plotS1(theta = "<1%", facet.by = "theta")
  th2 <- sub_plotS1(theta = "30%", facet.by = "theta")

  # asmyptomatic
  index_as <- sub_plotS1(facet.by = "prop.asym")
  as1 <- sub_plotS1(prop.asym = 0.1, facet.by="prop.asym")
...
make_figure_S2 <- function(res=NULL){
  ...
  dl1 <- sub_plotS2(delay = "Wuhan",facet.by = "delay")

  # theta
  index_th <- sub_plotS2(facet.by = "theta")

  th1 <- sub_plotS2(theta = "<1%",facet.by = "theta")

  th2 <- sub_plotS2(theta = "30%",facet.by = "theta")

  # asymptomatic
  index_as <- sub_plotS2(facet.by = "prop.asym")
  as1 <- sub_plotS2(prop.asym = 0.1,facet.by = "prop.asym")

  # compose plot

  pl <- ((patchwork::plot_spacer() + index_dl + dl1) /
           (th1 + index_th + th2) /
           (patchwork::plot_spacer() + index_as + as1)) + patchwork::plot_layout(guide="collect")
  ...
  pl & cowplot::theme_cowplot() &
    cowplot::panel_border() &
    ggplot2::theme(axis.title = ggplot2::element_blank()) &
    ggplot2::scale_fill_manual(values = c("dodgerblue","black","dodgerblue4"),name= "Number of\ninitial cases") &
    ggplot2::scale_colour_manual(guide="none",values = c("dodgerblue","black","dodgerblue4"))
...
make_figure_S3 <- function(res=NULL){
  ...
  th1 <- sub_plotS3(theta = "<1%",facet.by = "theta")

  th2 <- sub_plotS3(theta = "30%",facet.by = "theta")

  # asym
  index_as <- sub_plotS3(facet.by = "prop.asym")
  as1 <- sub_plotS3(prop.asym = 0.1,facet.by = "prop.asym")
...
make_figure_S4 <- function(res){
  ...
  pl <- index_as + as1 + patchwork::plot_layout(guides="collect")
