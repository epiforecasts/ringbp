...
make_figure_2a <- function(){
  ...
  out %>%
    ggplot2::ggplot(ggplot2::aes(x=x,y=value,ymin=0,ymax=value,fill=as.factor(dist))) +
    ...
    ggplot2::scale_fill_brewer(name="",labels=c("Short delay","Long delay"),palette="Set1") +
    ggplot2::scale_colour_brewer(guide="none",palette="Set1") +
    ggplot2::geom_vline(data=means,ggplot2::aes(xintercept=x,col=as.factor(dist)),lty=2,size=0.8) +
...
make_figure_2 <- function(){
  ...
  p2 <- data.frame(x=seq(0,15,0.1),y=dweibull(x=seq(0,15,0.1),shape = 2.322737,scale = 6.492272)) %>%
    ...
    ggplot2::geom_ribbon(aes(ymax=y,ymin=0),fill="chartreuse2",alpha=0.4)
  ...
  p3 <- data.frame(y=c(sn::dsn(x=seq(0,13,0.1),xi = 5,omega = 2,alpha = 0.7),
    ...
    ggplot2::theme(legend.position = "bottom") +
    ggplot2::scale_fill_manual(values=c("grey65","goldenrod3","orchid"),name="Proportion of\ntransmission\nbefore symptoms") +
    ggplot2::labs(tag = "C", x = "time since infection (days)",y = "probability density")
...
make_figure_3a <- function(df = NULL){
  pl <- df %>%
    ...
    ggplot2::ggplot(ggplot2::aes(x=control_effectiveness,y=pext,color=as.factor(index_R0))) +
    ggplot2::geom_line(size=0.75) +
    ggplot2::geom_point(shape=21,col="black",ggplot2::aes(fill=as.factor(index_R0)),size=3) +
    ggplot2::scale_fill_manual(guide="none",values = c("red","black","firebrick4")) +
    ...
    ggplot2::theme(legend.position = "bottom") +
    ggplot2::labs(tag = "A", x= "Contacts traced (%)", y = "Simulated outbreaks controlled (%)")
  return(pl)
}
...
make_figure_4 <- function(res = NULL){
  ...
  f4p1 <- res %>%
    dplyr::filter(delay == "SARS",
                  ...
                  index_R0 == 2.5,
                  prop.asym == 0) %>%
    ggplot2::ggplot(ggplot2::aes(x=control_effectiveness,y=pext,color=as.factor(num.initial.cases))) +
    ggplot2::geom_line(size=0.75) +
    ggplot2::geom_point(shape=21,col="black",aes(fill=as.factor(num.initial.cases)),size=3) +
    ggplot2::scale_fill_manual(guide="none",values = c("dodgerblue","black","dodgerblue3")) +
    ggplot2::scale_color_manual(values=c("dodgerblue","black","dodgerblue3"),name="Number of\ninitial cases")  +
    cowplot::theme_cowplot()
  ...
  f4p2 <- res %>%
    dplyr::filter(num.initial.cases==20,
                  ...
                  index_R0 == 2.5,
                  prop.asym == 0) %>%
    dplyr::mutate(delay=factor(delay,levels = c("SARS","Wuhan"),labels = c("Short","Long"))) %>%
    ggplot2::ggplot(ggplot2::aes(x=control_effectiveness,y=pext,color=as.factor(delay))) +
    ggplot2::geom_line(size=0.75) +
    ggplot2::geom_point(shape=21,col="black",aes(fill=as.factor(delay)),size=3) +
    ggplot2::scale_fill_manual(guide="none",values = c("black","forestgreen")) +
    ggplot2::scale_color_manual(values=c("black","forestgreen"), name="Onset to\nisolation\ndelay") +
    cowplot::theme_cowplot()
  ...
  f4p3 <- res %>%
    dplyr::filter(num.initial.cases==20,
                  ...
                  index_R0 == 2.5,
                  prop.asym == 0) %>%
    ggplot2::ggplot(ggplot2::aes(x=control_effectiveness,y=pext,color=as.factor(theta))) +
    ggplot2::geom_line(size=0.75) +
    ggplot2::geom_point(shape=21,col="black",aes(fill=as.factor(theta)),size=3) +
    ggplot2::scale_fill_manual(guide="none",values = c("mediumpurple2","black","mediumpurple4")) +
    ggplot2::scale_color_manual(values=c("mediumpurple2","black","mediumpurple4"), name = "Pre-symptom\ntransmission") +
    cowplot::theme_cowplot()
  ...
  f4p4 <- res %>%
    ...
    ggplot2::ggplot(aes(x=control_effectiveness,y=pext,color=as.factor(prop.asym))) +
    ggplot2::geom_line(size=0.75) +
    ggplot2::geom_point(shape=21,col="black",aes(fill=as.factor(prop.asym)),size=3) +
    ggplot2::scale_fill_manual(guide="none",values = c("black","chocolate4")) +
    ggplot2::scale_color_manual(values=c("black","chocolate4"),name="Proportion\nof cases\nwithout\nsymptoms")  +
    cowplot::theme_cowplot()
