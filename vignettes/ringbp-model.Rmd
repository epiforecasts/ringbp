...
\begin{align}
S_i &\ge 0 && \text{absolute symptom onset time}, \\
...
Define the relative symptom onset time as
\begin{equation}
\xi_i = S_i - E_i .
\end{equation}
...
The relative generation time $G_i^{\mathrm{rel}}$ is defined as the truncated
skew-normal random variable
\begin{equation}
G_i^{\mathrm{rel}} \sim X_i \mid X_i \ge L .
\end{equation}
...
The absolute generation time returned by the model is
\begin{equation}
G_i = G_i^{\mathrm{rel}} + E_i
\end{equation}
